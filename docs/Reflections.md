...
- ECS設計を取り入れたことで物理演算や描画を行うオブジェクトが増えても  
  カクつきが現れずに、快適なプレイ体験を提供できる。
- ECS設計を取り入れていない学校提供のゲームベースと比較して、  
  描画オブジェクト数が増える毎のFPSの変化は以下の結果が得られた。

![](images/ResultGraph.png)

- グラフと計測結果の詳細は[別リポジトリ](https://github.com/3doriTea/PerformanceGraph/tree/main)にまとめた。
...
- => しかし上記を別リポジトリで実装したところ、static 領域にインスタンスを置くことによる別の問題もあるため、  
  ゲームオブジェクトとしてシステムを登録する といった別の方法を模索中である。
